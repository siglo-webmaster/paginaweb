...
/*!50001 CREATE TABLE `view_ordenes_productos_proveedores` (
  ...
  `id_terceros_proveedores` tinyint NOT NULL,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_ordenes_productos_proveedores` AS select `op`.`id_ordenes` AS `id_ordenes`,`op`.`id_titulos` AS `id_titulos`,`op`.`id_tipos_productos` AS `id_tipos_productos`,`op`.`id_terceros_proveedores` AS `id_terceros_proveedores`,`tp`.`codigo` AS `codigo_producto`,`op`.`nombre_producto` AS `nombre_producto`,`op`.`cantidad` AS `cantidad` from (`ordenes_productos` `op` join `titulos_proveedores` `tp` on(((`tp`.`id_titulos` = `op`.`id_titulos`) and (`tp`.`id_terceros_proveedores` = `op`.`id_terceros_proveedores`)))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_titulos_destacados` AS select `d`.`id_destacados` AS `id_destacados`,`d`.`id_tipos_destacados` AS `id_tipos_destacados`,`td`.`nombre` AS `tipos_destacados_nombre`,`d`.`inicio` AS `inicio`,`d`.`fin` AS `fin`,`d`.`descuento` AS `descuento`,`d`.`observaciones` AS `observaciones`,`d`.`estado` AS `estado`,`tlp`.`id_titulos` AS `id_titulos`,`tlp`.`id_lista_precios` AS `id_lista_precios`,`tlp`.`id_terceros_proveedores` AS `id_terceros_proveedores`,`tlp`.`id_tipos_productos` AS `id_tipos_productos`,`tlp`.`precio` AS `precio` from (((`destacados` `d` join `tipos_destacados` `td` on((`td`.`id_tipos_destacados` = `d`.`id_tipos_destacados`))) join `titulos_destacados` `tid` on((`tid`.`id_destacados` = `d`.`id_destacados`))) join `titulos_lista_precios` `tlp` on((`tlp`.`id_titulos` = `tid`.`id_titulos`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

...
class classmodelSHEEventos{
    ...
    function geteventosActuales($tipo_evento=false){
        ...
        try{
            ...
            if(sizeof($this->data)>0){
                ...
                for($i=0;$i<(sizeof($this->data));$i++){
                    $conn_prepare = $this->conn->prepare($query);
                    $conn_prepare->bindParam(':id_eventos', $this->data[$i]['id_eventos'],PDO::PARAM_INT);
                    $conn_prepare->execute();
                    $this->data[$i]['detalles']= $conn_prepare->fetchAll();
                }

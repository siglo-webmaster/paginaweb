...
class classUsersAccounts {
    ...
    function loadModule(){
        ...
        if($this->model->confirmarPermisosModulo(array('id_usuarios'=>$_SESSION['userAdmin'],'id_modulos'=>$this->data['id_module']))){
    ...
    function login(){
        if((isset($this->data['user']))&&(isset($this->data['passwd']))){
            if($this->model->doLogin($this->data)!=false){
    ...
    function getMenuUser(){
        $this->model->getMenuUser(array('id_usuarios'=>$_SESSION['userAdmin']));
